...
%%%------------------------------------------%%%
% 04. Set Initial Temperature Fields and Energy Content
%%%------------------------------------------%%%
...
%%%------------------------------------------%%%
% 05. Time Loop over Scenario (t_900/2 for 10 days)
%%%------------------------------------------%%%
...
function [Res_900, LC, DC, Lflow, Dflow] = prepare_scenario_flow(Res_900, t_900, Nz, H, d)
    ...
    for k = 1:length(t_900)
        % Index of piston end in the vertical grid
    ...
    for k = 1:length(t_900)
        if t_900(1,k) > 0
            Res_900(9, k+1) = -Lflow * t_900(1,k);   % Charging velocity
        elseif t_900(1,k) < 0
            Res_900(10, k+1) = -Dflow * t_900(1,k);  % Discharging velocity
        end
    end
end
...
%   T_Sys  - full system state vector (including piston and vertical soil)
