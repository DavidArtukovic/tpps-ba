...
% 2. Bypass location
%%% ------------------------
z_bypass = 0.5;                              % [m] in each volume (local coordinate)
idx_b1   = round(z_bypass/dz) + 1;           % in upper water
idx_b2   = round(z_bypass/dz) + 1;           % in lower water
...
yline(z_bypass, 'k-.', 'LineWidth', 1);    % water volume 1 bypass
yline(H_w1+z_bypass, 'k-.', 'LineWidth', 1); % water volume 2 bypass
...
text(min(T_A)+5, 0, ...
     '\textit{water - insulation (external enthalpy source}', ...
     'Interpreter','latex', ...
     'VerticalAlignment','bottom');
...
text(min(T_A)+3, z_bypass-0.1, ...
     '\textit{bypass inlet}', ...
     'Interpreter','latex', ...
     'VerticalAlignment','bottom');

text(min(T_A)+5, H_w1+z_bypass, ...
     '\textit{bypass outlet}', ...
     'Interpreter','latex', ...
     'VerticalAlignment','bottom');