%% TEST: lambda_hybrid – extrapolated_zmix / downward / violation
% ---------------------------------------------------------------
% Tests the FK/FM lambda hybrid in an EXTRAPOLATED mixing case
% with DOWNWARD free convection where FK violates boundary conditions.
%
% Focus:
...
p = 0.15;                 % 0<p<1  (smaller => more "extremely" concave)
x = z./H;
...
assert(T_new(z_inlet_idx) <= T_old(z_inlet_idx) + tol, ...
    'Lambda hybrid did not fix inlet boundary violation (downward FK)');

assert(T_new(1) <= T_old(1) + tol, ...
    'Lambda hybrid did not fix bottom boundary violation (downward FK)');
...
assert(min(T_new(ids_mix)) >= Tin - tol, ...
    'Hybrid result undershot inlet temperature Tin');

% --- 4.4 Check energy conservation ---
rel_err = abs(E_new - (E_old + Q_add)) / max(abs(Q_add), tol);
assert(rel_err < tol, ...
    'Energy is not conserved in lambda extrapolated downward FK');


disp('✓ TEST PASSED: extrapolated_zmix / downward FK + lambda hybrid');
...
title('Extrapolated FK (downward) – lambda hybrid repair');
