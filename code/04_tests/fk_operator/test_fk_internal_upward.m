%% TEST: internal_zmix – upward FK
% ---------------------------------------------------------------
% Internal mixing height, upward free convection.
...
assert(min(T_new(ids_mix)) >= min(T_old(ids_mix)) - tol, ...
    'New minimum created in internal upward FK');

assert(max(T_new(ids_mix)) <= max(T_old(ids_mix)) + tol, ...
    'New maximum created in internal upward FK');

% --- 4.2 Heating at inlet ---
assert(T_new(z_inlet_idx) >= T_old(z_inlet_idx) + tol, ...
    'Inlet temperature decreased for internal upward FK');
...
assert(rel_err < tol, ...
    'Energy is not conserved in internal upward FK');

disp('✓ TEST PASSED: internal_zmix / upward FK');
...
title('Internal FK (upward) – profile comparison');
