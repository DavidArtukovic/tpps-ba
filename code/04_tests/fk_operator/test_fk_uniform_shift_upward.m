%% TEST: internal_zmix – upward FULLY MIXED
% ---------------------------------------------------------------
% Internal mixing height, upward fully mixed case.
% Entire mixing zone is shifted upward, warmer water enters.
% ---------------------------------------------------------------
...
% Mixing zone ABOVE inlet (upward)
z_mix_idx = Nz;
...
% --- 5.1 Heating anywhere in mixing zone ---
assert(all(T_new(ids_mix) >= T_old(ids_mix) - tol), ...
    'Cooling occurred in fully mixed upward case');

% --- 5.2 Energy conservation ---
rel_err = abs((E_new - E_old) - Q_add) / max(abs(Q_add), tol);

assert(rel_err < tol, ...
    'Energy is not conserved in upward fully mixed test');

disp('✓ TEST PASSED: upward FK - FULLY MIXED');
...
ylabel('z [m]');
legend('old profile','uniform shifted mixed profile','Location','best');
title('Internal upward fully mixed – profile comparison');
