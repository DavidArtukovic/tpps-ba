...
>>>>>>> be27bb7 (added exemplary plot with diffusion)
...
for dt = dt_list
    ...
    % Your closed-form update (kept as-is)
    T(ids) = 60 + (z(ids) - 10) * (1 - 9.2e-6 * dt);

    % Enforce Dirichlet BC after operator step
    T(1)   = T_left;
    T(end) = T_right;
