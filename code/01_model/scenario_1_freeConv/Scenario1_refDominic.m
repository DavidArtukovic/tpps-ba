...
load(fullfile(DATA_INIT, 'Init_56x226_pist.mat'));       % Actual init with modified flux
...
SW     = InitOut.SW;
% --- Extract state ---
IC_Sys = InitOut.IC_Sys;
T_REf  = InitOut.T_REf;
%t_900(1,900:1000) = 1; % Modify test array
% Result arrays
...
function [idx_bypass_lower_vec, idx_bypass_upper_vec, idx_membrane_vec] = compute_bypass_indices(soc_vec, h_pist, h_lift, Nz, dz)
    ...
    % This is given by s_crit = 1 - 8/h_lift = 0.5
    %s_crit = 1.0 - 8/h_lift;
    s_crit = 0.5;
    ...
    % Upper bypass is always (h_pist-h_lift)+h_lift/2 -> h_lift = h_pist -> upper bypass is h_lift/2 above lower bypass
    ...
    h_lift_off = ((h_pist-h_lift)/2)+h_lift;
    idx_membrane_vec = idx_rep_begin ...
    + round( ...
        Nrep/2 ...                          % mid at soc=1
      + (1 - soc_vec) .* (h_lift_off/h_pist) .* (Nrep/2) ...
    );
end
savefile = 'Test_900bis1200_56x226.mat';
save(savefile,"T_W_900","T_V_900","Res_900","IC_Sys")
