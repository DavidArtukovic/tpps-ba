...
function dTdt = HeatFluidSolid(t, T, Nz, dz, bypass_indices, flow, T0init, SW, A, z_RE, z_RP, alpha_w_eff)

    dTdt = zeros(Nz(10)+Nz(12)*Nz(13)+Nz(14)*Nz(3),1); 

    if nargin < 12 || isempty(alpha_w_eff)
        alpha_w_eff = SW(1,5) * ones(size(T));
    end
    ...
    for i = idx_w_bottom : idx_w_top
        ...
        if i <= Nz(3)+Nz(8)+Nz(2)-1
            ...
            alpha_ip = 0.5 * (alpha_w_eff(i) + alpha_w_eff(i+1));
            alpha_im = 0.5 * (alpha_w_eff(i) + alpha_w_eff(i-1));
            diffusion = ( alpha_ip * (T(i+1) - T(i)) ...
                        - alpha_im * (T(i)   - T(i-1)) ) / dz^2;
